$(window).load(function() {
    ...
    function hidePreloader() {
        ...
        preloader.delay(preloaderDelay).fadeOut(preloaderFadeOutTime, function() {
        	...
        	setTimeout ( function () {
        		$('link[rel=stylesheet][href~="theme/css/animate.css"]').remove();
    		},2000);
         });
    }
...
$(document).ready(function(){
	...
	if( !device.tablet() && !device.mobile() ) { //Load the animations if the device is not a mobile or tablet
		$('head').append('<!--[if !IE]><!--><link rel="stylesheet" type="text/css" media="screen" href="theme/css/animate.css"><!--<![endif]-->');
	}
    ...
    =============================================== */
	var now = new Date();
	var dateFinal = new Date("December 31, 2017 23:59:59");// new Date( year , month , day , hour , minutes , seconds)
	var difference = parseInt(dateFinal-now); //fix a gap of 30 days
	var countTo = parseInt(difference + now.valueOf());
	...
	if ( ($('.minutes').html()) == 1 ){
		$('.minutesText').text(" phút ");
	}else{
		$('.minutesText').text(" phút ");
	}
	
	if ( ($('.seconds').html()) == 1 ){
		$('.secondsText').text(" giây ");
	}else{
		$('.secondsText').text(" giây ");
	}
